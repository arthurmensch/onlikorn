...
The Wasserstein cost \todo{Since $\epsilon,C$ are fixed everywhere, maybe we could rename $\Ww_{C,\epsilon}$ are $\Ww$ (and name $\Ww_C$ differently) and $T$ instead of $T_{C,\epsilon}$?}  between two probability distributions $(\alpha, \beta) \in \Pp(\Xx)^2$ is defined as the minimal cost required to move each element of mass of $\alpha$ to each element of mass of $\beta$. It rewrites as the solution of a
...
%
The regularized problem~\eqref{eq:wass} admits a dual form, which is a maximization problem over the space of continuous \textit{potential} functions:
...
It can be shown that ${(f_t)}_t$ and ${(g_t)}_t$ converge in $(\Cc(\Xx)/\sim,
...
Although consistency is a reassuring
