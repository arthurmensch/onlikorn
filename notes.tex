...
\end{align}
We know use the three point propery:
\begin{equation}
...
\end{align}
Set $\nu = \nu_k$, $\mu_z = \mu_{k}$ $\mu = \mu_{k+1}$, $\nu_z = \nu_k$, we obtain a descent lemma
\begin{equation}
    F(\mu_{k+1}, \nu_{k+1}) \leq F(\mu_{k+1}, \nu_k) \leq F(\mu_k, \nu_k).
\end{equation}
Choosing an optimal couple $\mu^\star, \nu^\star$, setting $\nu = \nu^\star$, $\mu = \mu^\star$, and summing both equations over $k$, we obtain
\begin{equation}
    F(\mu_{K+1}, \nu^\star) + 
