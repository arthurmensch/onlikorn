...
${(y_i^s)}_{i,t} \in~\Yy, {(x_i^s)}_{i,s\leq t} \in~\Xx$ \todo{Lacking $1/\epsilon$?}
\begin{align}
...
Provided with fresh samples $(x_i^t)_i$ and $(y_i^t)_i$, a naive approach updates the potentials using a noisy soft $C$-transform
\begin{equation}
...
The variance introduced must therefore be reduced in some way---in other words, we cannot afford to forget past iterates to obtain a consistent estimation of potentials. We introduce a learning rate in Sinkhorn iterations, that averages the past representations and the newly computed noisy C-transforms, \todo{Lacking $1/\epsilon$?}
\begin{align}
...
compute out-of-loop averages of potentials\todo{Lacking $1/\epsilon$?}
\begin{align}
