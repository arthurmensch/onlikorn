...
show that online Sinkhorn accurately estimates correctly the solutions of
...
We first consider discrete distributions $(\alpha, \beta)$, to be able to
compute reference distance $\Ww(\alpha, \beta)$ and potentials $f^\star$,
$g^\star$, using Sinkhorn's algorithm. The goal here is not to perform better
...
C(x,y)=\norm{x-y}^2$ and use $\eta_t = \frac{1}{\sqrt{t}}$ for online Sinkhorn,
in all experiments. We compare the performance of Sinkhorn, online Sinkhorn and
...
computation units. We further report the curves using out-of-loop, $\gamma_t =
\frac{1}{\sqrt{t}}$ averaging. 
