...
The Sinkhorn algorithm requires to compute a full matrix $C_d \eqdef (C(x_i,y_i))_{i,j}$ prior to estimating
...
upon Sinkhorn algorithm. After \todo{what is $N$?} $N^2$ computation (the cost of estimating the full matrix $C_d$),
