...
not a Hilbert norm. The constant $A$ depends on $L$, the diameter of $\Xx$ and the regularity of potentials $f^\star$ and $g^\star$, but not on the dimension. It behaves like $\exp(\frac{1}{\varepsilon})$ when the
$\varepsilon \to 0$.
...
$D$ is proportional to $A \exp(\frac{1}{\epsilon})$, and therefore explodes for
