...
\if\icml1
\subsection{Recapitulation on batch-sizes and learning rates}
...
\begin{figure}[t]
    ...
    \includegraphics[width=\linewidth]{online_0.1_False_test}
    
    $\varepsilon = 0.01$

    \includegraphics[width=\linewidth]{online_0.01_False_test}

    $\varepsilon = 0.001$

    \includegraphics[width=\linewidth]{online_0.001_False_test}

    $\varepsilon = 0.0001$

    \includegraphics[width=\linewidth]{online_0.0001_False_test}
    \end{widepage}
...
\begin{figure}[t]
    ...
    \includegraphics[width=\linewidth]{online_0.1_True_test}
    
    $\varepsilon = 0.01$

    \includegraphics[width=\linewidth]{online_0.01_True_test}

    $\varepsilon = 0.001$

    \includegraphics[width=\linewidth]{online_0.001_True_test}

    $\varepsilon = 0.0001$

    \includegraphics[width=\linewidth]{online_0.0001_True_test}
    \end{widepage}

    \caption{Performance of fully-corrective online Sinkhorn (O-S) for various $\varepsilon$.}
...
\begin{figure}[t]
    \begin{widepage}
    \centering
    \includegraphics[width=\linewidth]{random_0.1_test_}
    
    $\varepsilon = 0.01$

    \includegraphics[width=\linewidth]{random_0.01_test_}

    $\varepsilon = 0.001$

    \includegraphics[width=\linewidth]{random_0.001_test_}

    $\varepsilon = 0.0001$

    \includegraphics[width=\linewidth]{random_0.0001_test_}
    \end{widepage}

    \caption{Performance of randomized Sinkhorn (R-S) for various $\varepsilon$.}
...
\paragraph{All OS convergence curves.} To complete \autoref{fig:convergence},
...
performance of randomized Sinkhorn. In low dimension, randomized Sinkhorn is a
reasonable alternative to batch Sinkhorn, as it often outperforms it on average,
for the same memory complexity (compare purple to orange curve for instance). In
high dimension, batch Sinkhorn tend to perform slightly better.
...
We compare batch Sinkhorn $(N=100, 1000, 10000)$ to (non fully-corrected) online
...
\begin{figure}[t]
    \begin{widepage}
        ...
        \includegraphics[width=\linewidth]{online_0.1_False_test_gaussian}
        
        $\varepsilon = 0.01$
        
        \includegraphics[width=\linewidth]{online_0.01_False_test_gaussian}
    
        $\varepsilon = 0.001$
    
        \includegraphics[width=\linewidth]{online_0.001_False_test_gaussian}
    
        $\varepsilon = 0.0001$
    
        \includegraphics[width=\linewidth]{online_0.0001_False_test_gaussian}
    \end{widepage}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=\linewidth]{quiver_os_1e8}

    \includegraphics[width=\linewidth]{quiver_subsampled_1e8}

    \includegraphics[width=\linewidth]{quiver_ref}
    
    \caption{Displacement field as defined by the potentials estimated by online-Sinkhorn and Sinkhorn on a 2D GMM. With the same computational budget, online Sinkhorn finds smoother displacement fields than Sinkhorn. Those are closer to the true reference displacement field (we use Sinkhorn on $N=10000$ to estimate this reference). $\alpha$ and $\beta$ log-likelihood level-lines are displayed in red and blue, while the arrows are proportional to $\nabla_x \hat f_t(x) \d \alpha(x)$.}
...
backpropagation. For simplicity, let us assume that $\alpha = \frac{1}{n} \sum_{i=1}^n \delta_{x_i}$. Then, for all $i \in [1, n]$,
\begin{equation}
    \frac{\partial \Ww(\alpha, \beta)}{\partial x_i} = \nabla_x \big(x \to f^\star(\alpha, \beta)\big) (x_i),
\end{equation}
...
$(N=1,000)$, for the same number of computations.
...
\subsection{Online Sinkhorn as a warmup process}\label{app:warmup_exp}
...
\begin{figure}[t]
    ...
    \includegraphics[width=\linewidth]{online+full_0.1_err}
    
    $\varepsilon = 0.01$

    \includegraphics[width=\linewidth]{online+full_0.01_err}

    $\varepsilon = 0.001$

    \includegraphics[width=\linewidth]{online+full_0.001_err}

    $\varepsilon = 0.0001$

    \includegraphics[width=\linewidth]{online+full_0.0001_err}
