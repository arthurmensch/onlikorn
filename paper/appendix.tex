...
quantities on the batch size $n$. Using \autoref{lemma:lln}, we bound the quantity
\begin{align}
...
\autoref{lemma:lln} ensures that there exists $A$ (that depends only on $\beta$)
such that
\begin{equation}
    E_n \leq \frac{A}{\sqrt{n}}.
\end{equation}
We now bound $\EE_{\hat \beta_n} {|| \epsilon_{\hat \beta_n} ||}_\var$ using the
 quantity $E_n$. First, we observe that there exists $g^\star_{\min},
 g^\star_{\max}, C_{\max} \in \RR$ such that $g^\star_{\min} \leq g^\star \leq
 g^\star_{\max}$ on $\Xx$ and $C(x, y) \leq C_{\max}$ for all $x, y \in \Xx$, thanks to the
 \autoref{ass:lip}. Therefore
 \begin{align}
    \delta \triangleq 
    \exp(g^\star_{\min} - C_{\max}) &\leq \exp(-T_\beta(g^\star)) \leq \exp(g^\star_{\max}) \\
    \delta &\leq \exp(-T_{\hat \beta_n} 
    (g^\star)) \leq \exp(g^\star_{\max}).
\end{align}
For all $x \in \Xx$, 
\begin{equation}
    | \epsilon_{\hat \beta_n}(x) | = 
    | \log \frac{\exp(-T_{\beta_n}(g^\star))(x)}{\exp(-T_\beta(g^\star))(x)} |,
\end{equation}
so that
...
\end{equation}
 Then, we note that, for $n$ large enough, $E_n < \delta$, and for all $x \in \Xx$, 
\begin{align}\label{eq:log_ineq}
    {|| \varepsilon_{\hat \beta_n} ||}_{\var} \leq | \epsilon_{\hat \beta_n}(x) |
    &= | \log(1 + 
    \frac{\exp(-\Ctrans{g^\star}{\beta})
    - \exp(-\Ctrans{g^\star}{\hat \beta_n})}{\exp(-\Ctrans{g^\star}{\beta}})(x) | \\
    &\leq \max ( \log(1 + \frac{E_n}{\delta}),
     - \log(1 - \frac{E_n}{\delta}) ) \\ 
    &= - \log(1 - D_n),
\end{align}
where we have set $D_n \triangleq \frac{E_n}{\delta}$. On the event $\{D_n \leq
\frac{1}{2}\}$, a simple calculation gives $- \log(1 - D_n) \leq (2 \log 2)
D_n$. Thanks to Markov inequality, $\PP[D_n > \frac{1}{2}] \leq 2 \EE[D_n]$.

We then split the expectation over the previous event, and use inequalities \eqref{eq:log_ineq} and \eqref{eq:const_ineq}
on each conditional expectation:
\begin{align}\label{eq:vdv}
    \EE {|| \varepsilon_{\hat \beta_n} ||}_{\var}  &= \PP\left[D_n \leq \frac{1}{2}\right] 
    \EE \left[ {|| \varepsilon_{\hat \beta_n} ||}_{\var}
    \Big| D_n \leq \frac{1}{2}\right] 
    \\
    &\phantom{=}
    + \PP\left[D_n > \frac{1}{2}\right]      \EE \left[ {|| \varepsilon_{\hat \beta_n} ||}_{\var}
    \Big| D_n > \frac{1}{2}\right] \\
    &\leq (\frac{2 \log 2}{\delta} + 2 {\Vert g^\star \Vert}_{\var} + C_{\max}) \EE[E_n] \leq \frac{C'}{\sqrt{n}},
\end{align}
where $A'$ is a constant depending on $\alpha, \beta$ and $C$. A similar results
holds for $\EE_{\hat \alpha_n} {\Vert \iota_{\hat \alpha_n} \Vert}_{\var}$.
Therefore, there exists $A'' > 0$ such that $r_\infty \leq \frac{A''}{\sqrt{n}}$.
As for all $t >0$, $e_t \leq r_t \to_{t \to \infty} r_\infty$, the proposition follows.
...
large number \eqref{eq:vdv}, there exists $A, A' > 0$ such that
\begin{align}\label{eq:recursion}
    \EE e_{t+1} &\leq (1 - \tilde \eta_t) \EE e_t + \tilde \eta_t \frac{A}{(1 - \kappa) \sqrt{n(t)}} \\
    &\leq (1 - \tilde \eta_t) \EE e_t + A' \tilde \eta_t w_t,
\end{align}
...
\begin{equation}\label{eq:Et_rec2}
    \EE e_{t+1} - \EE e_t = - \tilde \eta_t \EE e_t + A' \tilde \eta_t w_t \leq A' \tilde \eta_t w_t
\end{equation}
...
\begin{equation}
    ...
    + A' \sum_{s=1}^{t-1} \tilde \eta_s w_s \to_{t \to \infty} - \infty,
\end{equation}
